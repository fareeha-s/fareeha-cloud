...
const scattered = [
  { photo: polaroidPhotos.scrumptious, style: { left: '-9vw', top: '3vh' }, rotate: -9 },
...
// `recede`: when an app is open the wallpaper blurs, dims and zooms in, the way iOS
// pushes the home screen back, so text on top stays easy to read.
const MobileWallpaper: React.FC<{ isLoaded: boolean; recede: boolean }> = ({ isLoaded, recede }) => {
  ...
  return (
    <div
      ...
      aria-hidden="true"
    >
      <div className={`mobile-wallpaper absolute inset-0 ${recede ? 'mobile-wallpaper--receded' : ''}`}>
